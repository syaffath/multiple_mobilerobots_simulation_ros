...
pub_speed_x = rospy.Publisher("/robot_Leader/speed_x", Float32, queue_size=1)
pub_speed_z = rospy.Publisher("/robot_Leader/speed_z", Float32, queue_size=1)
...
Kp_lin = 0.5 #0.5 60cm,-90 #0.2 20cm,0
Ki_lin = 0.0
Kd_lin = 1.0 #0.2
...
while not rospy.is_shutdown():
    ...
    setpoint.x = (xd-xo)*(t_n/t_cmd) + xo
    setpoint.y = A*math.sin(math.pi * (setpoint.x-xo) / k1) + yo
    ...
    pub.publish(speed)
    pub_x_leader.publish(x)
    pub_y_leader.publish(y)
    pub_theta_leader.publish(theta_degree)

    pub_speed_x.publish(speed_x)
    pub_speed_z.publish(speed_z)
#    print (t_n)
